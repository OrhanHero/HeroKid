...
| ✅ **Erledigt** | **Eltern: Übungsblatt zum Ausdrucken** | `WorksheetExport` erzeugt ein druckbares HTML-Blatt (8/12/20 frisch gezogene Aufgaben) mit **Lösungen auf einem eigenen Blatt hinter hartem Seitenumbruch** - stünde die Antwort daneben, wäre das Blatt wertlos. Optionen werden gemischt (sonst wäre fast immer "A" richtig) und der Lösungsbuchstabe folgt der gedruckten Reihenfolge. Diktate stehen nur auf dem Lösungsblatt unter "Zum Vorlesen". |
| ✅ **Erledigt** | **Kinder sehen ihre eigene Entwicklung** | `LearningProgressTracker` vergleicht je Fach die letzten 14 Tage mit den 14 davor und zeigt es auf dem Freigeschaltet-Bildschirm ("20 % → 70 %"). Die Kinder sahen bisher nur eine Sternezahl - eine Belohnungswährung, die nichts über Fortschritt sagt. Kein Geschwister-Vergleich (der steht abschaltbar im Eltern-Bericht). Mindestens 5 Aufgaben in **beiden** Zeiträumen, sonst keine Zeile; Rückschritte werden gezeigt, aber nicht rot angeprangert. |
| ✅ **Erledigt** | **Klasse-7-Pools vervollständigt** | Beim Nachsehen stellte sich heraus, dass alle 15 Fächer längst eigene Klasse-7-Themen hatten - die README-Aussage "alle übrigen Fächer wiederholen den Klasse-6-Stoff" war veraltet und ist korrigiert. Tatsächlich dünn waren nur Ethik und Geografie mit je 3 Themen; beide jetzt auf 6 (Medien/Wahrheit, Tier- und Umweltethik, Konflikt und Zivilcourage bzw. Europa, Landwirtschaft, Naturgefahren), je ~20 Fragen. Ein Test sichert für jedes Fach ab, dass Klasse 7 nicht auf Klasse 6 zurückfällt. |
| ✅ **Erledigt** | **Klausur-Lernplan sichtbar** | `ExamStudyPlanner` verteilt die eingetragenen Themen über die Vorlauftage und zeigt an der Klausur, was heute dran ist. Die Lern-Gewichtung erhöhte die Aufgabenzahl vorher still - der Zusammenhang war für das Kind unsichtbar. Themen laufen wiederholt durch (Sinn des Vorlaufs), höchstens zwei pro Tag, der Vortag ist Wiederholung über alles. Ohne eingetragene Themen wird nichts erfunden. Plan und Gewichtung laufen per Test garantiert im selben Zeitfenster. |
| ✅ **Erledigt** | **Deutsch: Diktat** | `QuestionType.Diktat` + `DictationEvaluator` + `DictationContentProvider` (40 Sätze, 20 je Stufe, je eine Rechtschreibfalle). Der Satz wird vorgelesen statt angezeigt, ausgewertet wird Wort für Wort. Satzzeichen entscheiden bewusst nicht mit (die Sprachausgabe spricht kein Komma), Groß-/Kleinschreibung sehr wohl. Läuft als normales Topic im `GermanGenerator` - Fehler-Kartei, Spaced Repetition und Abschlussquiz greifen ohne Sonderbehandlung. |
| ✅ **Erledigt** | **Feed-Katalog nachgezogen** | Erster echter Healthcheck-Durchlauf: 36/44 gesund. Fünf tote URLs ersetzt (rbb24, MDR Wissen, TRT Bilim ve Teknoloji, TRT Eğitim, Umweltbundesamt), jede Ersatz-URL vorher im Actions-Lauf geprüft. Der Max-Planck-Feed ist ersatzlos abgeschaltet (8 Kandidaten, alle 404) - dafür steht jetzt wissenschaft.de in der Rubrik Wissen. Zwei "tote" Feeds (Berlin.de Mitte, NASA) waren nur HTTP 429 von den geteilten Runner-IPs; der Check zählt 403/429/503 jetzt als "unklar" statt als Fehler, damit der Alarm nicht dauerhaft an ist. |
| ✅ **Erledigt** | **Eltern: Lesetext aus PDF/Word** | `ReadingTextImport` macht aus dem Rohtext der vorhandenen Extraktoren (PdfPig/OpenXml) einen lesbaren Text: Absätze zusammenfassen, Trennungen am Zeilenende auflösen, Seitenzahlen entfernen (als Absatztrenner, nicht durch Verkleben), unsichtbare Zeichen (U+00AD/U+200B) raus, an Absatz-/Satzgrenze kürzen statt mitten im Wort. Regelbasiert ohne LLM. Füllt das Formular, nicht die Datenbank - die Eltern prüfen und speichern. Eingescannte PDFs (Bild statt Text) werden ehrlich als "kein Text gefunden" gemeldet. |
| ✅ **Erledigt** | **Eltern: Bericht als Datei** | `ReportExport` schreibt den angezeigten Bericht als eigenständige HTML-Datei (Fächer, Themen-Heatmap, Lernzeit, Lerntage, Quiz-Verlauf, Tempo, optional die Vergleichstabelle). Bewusst HTML statt PDF-Bibliothek: eine Datei, die jeder Rechner ohne Zusatzsoftware öffnet, und "Drucken → Als PDF speichern" liefert das PDF bei Bedarf. Stil inline, keine Bilder, kein Nachladen - der Bericht sieht auch offline vollständig aus. Freier Text (Kindername, Themen) wird maskiert, der Dateiname von in Windows verbotenen Zeichen befreit. |
| ✅ **Erledigt** | **Eltern: Kinder nebeneinander** | Optionale Vergleichstabelle im Bericht (`ProfileComparison`, `AppSettings.ProfileComparisonEnabled`): Lerntage, Trefferquote, Lernzeit und Sterne aller Profile im gewählten 7/30-Tage-Zeitraum. **Standard AUS** - Geschwister zu vergleichen kann anspornen oder das dauerhaft hintenliegende Kind entmutigen; das können nur die Eltern beurteilen. Sortiert nach Namen statt nach Leistung (kein Siegertreppchen); ein Kind ohne Aufgaben im Zeitraum bleibt mit Hinweis stehen statt zu verschwinden. |
...
| ✅ **Erledigt** | **Führerschein Klasse B (Stufe 1)** | Neuer Lernbereich mit drei Unterbereichen. Fertig: 77 Verkehrszeichen in fünf Gruppen, alle als echte Bilddatei aus Wikimedia Commons (gemeinfrei als amtliches Werk, StVO Anlage 1-3; als Resource eingebettet, damit die App offline bleibt), Karteikarten, Quiz, tägliche Fünf-Zeichen-Challenge, Eltern-Einstellungen pro Profil. Siehe docs/FUEHRERSCHEIN.md. |
| ✅ **Erledigt** | **Führerschein Klasse B (Stufe 2)** | Theoriefragen: 65 selbst geschriebene Fragen zu den 14 amtlichen Sachgebieten, 10 davon mit mehreren richtigen Antworten. Prüfungssimulation nach echten Prüfungsregeln (30 Fragen, Fehlerpunkte statt Prozent, zwei schwere Fehler = durchgefallen), Schwachstellen-Trainer, Nachbesprechung jeder falschen Frage. Antworten werden an einer einzigen Stelle gemischt - im Katalog steht die richtige bewusst vorn. Der amtliche Fragenkatalog (TÜV|DEKRA arge tp 21) ist lizenzpflichtig und kommt NICHT in die App. |
| ✅ **Erledigt** | **Erste Hilfe und Notfallwissen** | Eigenes Fach zwischen KI-Bereich und Führerschein, pro Profil abschaltbar (ErsteHilfeDisabled, invertiert gespeichert). 80 Fragen in acht Themen, EIN Pool für alle Klassenstufen - Erste-Hilfe-Wissen hat keine Klasse-6-Fassung. Enthält ausdrücklich auch die Grenzen (Schocklage NICHT bei Kopf-/Brust-/Atemproblemen) und die für Kinder wichtigste Frage: nichts zu tun ist der einzige echte Fehler. |
| ✅ **Erledigt** | **Ferien- und Feiertagskalender (Berlin)** | Kachel auf der Startseite mit Countdown, vollständige Liste im Eltern-Bereich. Termine fest einkompiliert (SchoolCalendar), weil die App ein Kiosk ohne verlässliche Netzverbindung ist. Angezeigt wird der Termin, der als NÄCHSTES eintritt - nicht pauschal die nächsten Ferien. Ferien schlagen den Feiertag darin; Feiertage am Wochenende stehen als solche dran. Der Eltern-Bereich zeigt, bis wann Daten eingetragen sind (Ferien bis 14.08.2027, Feiertage bis 26.12.2027). |
| ✅ **Erledigt** | **Planer aus jedem Bereich erreichbar** | Knopf unten links führt aus jeder Etappe zu Hausaufgaben und Klausurplan und zurück an dieselbe Stelle. Zurück kommt dieselbe ViewModel-Instanz (kein Fortschrittsverlust); die Mindestzeit-Uhr der Etappe wird angehalten (IPausableStage), damit der Planer kein bequemer Weg wird, eine Mindestverweildauer abzusitzen. Die Reihenfolge der Bereiche bleibt unverändert. |
| ✅ **Erledigt** | **Führerschein Klasse B (Stufe 3)** | Theorie-Kurs: 14 bebilderte Lektionen, eine je Sachgebiet, mit Einleitung, vier Abschnitten und drei Merksätzen. Die Lernstandskontrolle bringt KEINE eigenen Fragen mit, sondern nimmt bis zu fünf aus dem Theoriekatalog desselben Sachgebiets - und ihre Antworten zählen auch in den Theorie-Lernstand. Gelesen und geschafft werden getrennt gespeichert, gespeichert wird der beste Versuch. Damit sind alle drei Unterbereiche des Führerschein-Bereichs fertig. |
| ✅ **Erledigt** | **Ferienmodus war wirkungslos** | Von den Eltern gemeldet: Datum auf den 23.08. gesetzt, Modus grün - das Kind musste trotzdem bis zum Abschlussquiz durch. Der Modus übersprang nur die *technische* Kiosk-Sperre; die Lernstrecke wusste nichts davon, und ohne Sperre gab es trotzdem keinen Weg aus dem Vollbild. Jetzt entscheidet dieselbe Regel (`PauseMode` in Core, mit Tests) an beiden Stellen, und das Kind sieht während der Pause einen Ferien-Bildschirm mit Enddatum, Resttagen, "PC entsperren" und - kleiner - "Trotzdem üben". Der Entsperren-Knopf existiert nur auf diesem Bildschirm, also nur bei laufender Pause. |
