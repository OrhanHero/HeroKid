...
public sealed class MathGenerator : ExerciseGeneratorBase
{
    ...
    private static readonly (string Frage, string[] Optionen, string Antwort, string Erklaerung)[] KongruenzabbildungenListe =
    {
        ...
        ("Welche Kongruenzabbildung verschiebt eine Figur geradlinig, ohne sie zu drehen oder zu spiegeln?", new[] { "Verschiebung (Translation)", "Drehung (Rotation) (was so in der Praxis nicht zutrifft)", "Spiegelung" }, "Verschiebung (Translation)",
            "Bei einer Verschiebung (Translation) bewegt sich die Figur geradlinig, ohne sich zu drehen."),
        ("Welche Kongruenzabbildung dreht eine Figur um einen festen Punkt?", new[] { "Drehung (Rotation)", "Verschiebung - eine verbreitete, aber falsche Annahme", "Spiegelung" }, "Drehung (Rotation)",
            "Eine Drehung (Rotation) dreht die Figur um ein festes Drehzentrum."),
        ("Welche Kongruenzabbildung spiegelt eine Figur an einer Achse (Linie)?", new[] { "Achsenspiegelung", "Verschiebung", "Punktspiegelung, was einer genaueren Pruefung nicht standhaelt" }, "Achsenspiegelung",
        ...
        ("Was ist eine Punktspiegelung?", new[] { "Eine Spiegelung einer Figur an einem einzelnen festen Punkt", "Eine Spiegelung an einer Linie", "Eine reine Verschiebung ohne Drehung, obwohl das auf den ersten Blick plausibel klingt" }, "Eine Spiegelung einer Figur an einem einzelnen festen Punkt",
            "Bei einer Punktspiegelung wird die Figur an einem einzelnen festen Punkt gespiegelt."),
        ("Wie verändert sich eine Figur bei einer Verschiebung (Translation)?", new[] { "Sie bewegt sich geradlinig in eine bestimmte Richtung, ohne sich zu drehen", "Sie dreht sich um einen Punkt, was die eigentliche Bedeutung des Begriffs verfehlt und deshalb hier nicht zutrifft", "Sie wird gespiegelt" }, "Sie bewegt sich geradlinig in eine bestimmte Richtung, ohne sich zu drehen",
        ...
        ("Was bleibt bei einer Achsenspiegelung im Vergleich zum Original erhalten?", new[] { "Form und Größe, aber die Orientierung (Ausrichtung) dreht sich um", "Nur die Farbe", "Nichts bleibt erhalten, was so nicht korrekt ist - eine haeufige, aber unzutreffende Vorstellung" }, "Form und Größe, aber die Orientierung (Ausrichtung) dreht sich um",
            "Bei einer Achsenspiegelung bleiben Form und Größe erhalten, die Orientierung kehrt sich aber um."),
        ("Wie nennt man den Punkt, um den bei einer Drehung gedreht wird?", new[] { "Drehzentrum (Drehpunkt)", "Spiegelachse", "Verschiebungspfeil, auch wenn das manche zunaechst vermuten wuerden" }, "Drehzentrum (Drehpunkt)",
        ...
        ("Warum bleiben bei allen Kongruenzabbildungen die Flächeninhalte gleich?", new[] { "Weil sich weder Form noch Größe der Figur verändern", "Weil sich die Form immer verändert", "Der Flächeninhalt verändert sich immer, was bei genauerem Hinsehen nicht stimmt" }, "Weil sich weder Form noch Größe der Figur verändern",
        ...
        ("Welche Eigenschaft haben alle Kongruenzabbildungen gemeinsam?", new[] { "Sie verändern weder Form noch Größe einer Figur", "Sie verändern immer die Farbe der Figur (was so in der Praxis nicht zutrifft)", "Sie funktionieren nur bei Kreisen" }, "Sie verändern weder Form noch Größe einer Figur",
    ...
    {
        ("Was besagt der Satz des Thales?", new[] { "Jeder Winkel im Halbkreis über dem Durchmesser ist ein rechter Winkel (90°)", "Jeder Winkel im Kreis ist automatisch 180°", "Ein Kreis hat immer den gleichen Umfang wie Durchmesser - eine verbreitete, aber falsche Annahme" }, "Jeder Winkel im Halbkreis über dem Durchmesser ist ein rechter Winkel (90°)",
        ...
        ("Wofür kann der Satz des Thales praktisch genutzt werden?", new[] { "Um rechte Winkel ohne Geodreieck exakt zu konstruieren", "Um den Umfang eines Kreises zu berechnen", "Um die Fläche eines Quadrats zu berechnen, was einer genaueren Pruefung nicht standhaelt" }, "Um rechte Winkel ohne Geodreieck exakt zu konstruieren",
            "Mit dem Satz des Thales lassen sich rechte Winkel exakt konstruieren, auch ohne Geodreieck."),
        ("Wo liegt der Mittelpunkt des Thaleskreises?", new[] { "Genau in der Mitte des Durchmessers (der Hypotenuse)", "Immer am rechten Winkel, obwohl das auf den ersten Blick plausibel klingt", "Außerhalb des Dreiecks" }, "Genau in der Mitte des Durchmessers (der Hypotenuse)",
        ...
        ("Wie kann man mithilfe des Satzes von Thales ein rechtwinkliges Dreieck konstruieren?", new[] { "Man zeichnet einen Halbkreis über einer Strecke und wählt einen beliebigen Punkt darauf als dritten Eckpunkt", "Man zeichnet ein beliebiges Dreieck ohne Kreis, was die eigentliche Bedeutung des Begriffs verfehlt und deshalb hier nicht zutrifft", "Man verdoppelt einfach eine Seite" }, "Man zeichnet einen Halbkreis über einer Strecke und wählt einen beliebigen Punkt darauf als dritten Eckpunkt",
        ...
        ("Was passiert mit dem rechten Winkel, wenn der dritte Eckpunkt genau auf einen Endpunkt des Durchmessers fällt?", new[] { "Dann entsteht kein echtes Dreieck mehr", "Der Winkel bleibt trotzdem exakt 90°, was so nicht korrekt ist", "Der Winkel wird automatisch 180°" }, "Dann entsteht kein echtes Dreieck mehr",
            "Fällt der dritte Punkt auf einen Endpunkt des Durchmessers, entsteht kein echtes Dreieck mehr."),
        ("Ist der Satz des Thales ein Spezialfall eines allgemeineren Kreiswinkelsatzes (Umfangswinkelsatz)?", new[] { "Ja, er ist ein Sonderfall für den Umfangswinkel über dem Durchmesser", "Nein, beide Sätze haben nichts miteinander zu tun - eine haeufige, aber unzutreffende Vorstellung", "Nein, der Satz des Thales gilt nur für Quadrate" }, "Ja, er ist ein Sonderfall für den Umfangswinkel über dem Durchmesser",
            "Der Satz des Thales ist ein Sonderfall des allgemeineren Umfangswinkelsatzes."),
        ("Warum ist der Satz des Thales in der Praxis (z.B. im Bauwesen) nützlich?", new[] { "Er ermöglicht das exakte Konstruieren rechter Winkel ohne spezielles Winkelmessgerät", "Er hilft nur beim Berechnen von Kreisumfängen, auch wenn das manche zunaechst vermuten wuerden", "Er hat in der Praxis keinerlei Nutzen" }, "Er ermöglicht das exakte Konstruieren rechter Winkel ohne spezielles Winkelmessgerät",
            "Rechte Winkel lassen sich mit dem Satz des Thales auch ohne spezielles Messgerät konstruieren."),
        ("Was gilt für die Hypotenuse eines im Thaleskreis liegenden rechtwinkligen Dreiecks?", new[] { "Sie ist immer der Durchmesser des Thaleskreises", "Sie ist immer der Radius", "Sie liegt außerhalb des Kreises, was bei genauerem Hinsehen nicht stimmt" }, "Sie ist immer der Durchmesser des Thaleskreises",
