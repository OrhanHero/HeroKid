...
/// <summary>
/// Gemeinsamer HTTP-Client für die NotebookLM-Enterprise-API (Google Cloud) - kapselt den kompletten
/// Wegwerf-Notebook-Lebenszyklus (anlegen, Text als Quelle hochladen, Frage stellen, wieder löschen),
/// den sowohl <see cref="TeacherImport.NotebookLmQuestionSuggester"/> (Lehrer-Import) als auch der
/// NotebookLM-Anbieter des KI-Lernchats identisch brauchen - vorher in
/// <c>NotebookLmQuestionSuggester</c> dupliziert, jetzt einmal hier.
///
...
/// </list>
/// </summary>
public sealed class NotebookLmClient
{
    ...
    public NotebookLmClient(HttpClient httpClient, NotebookLmOptions options)
    {
    ...
    /// <summary>
    /// Legt ein Wegwerf-Notebook an, lädt <paramref name="sourceText"/> als einzige Quelle hoch,
    /// stellt <paramref name="prompt"/> als Frage und löscht das Notebook danach wieder - ein
    /// kompletter Frage-Antwort-Zyklus in einem Aufruf. <paramref name="notebookTitle"/> dient nur der
    /// Übersicht in der Google-Cloud-Konsole, hat keine funktionale Bedeutung.
    /// </summary>
    public async Task<string> AskAsync(string notebookTitle, string sourceText, string prompt, CancellationToken cancellationToken)
    {
        if (!_options.IsConfigured)
        {
            throw new NotSupportedException(
                "NotebookLM ist nicht konfiguriert (Projekt-Nummer und/oder Dienstkonto-Schlüsseldatei " +
                "fehlen im Eltern-Bereich unter 'Automatisches Einlesen').");
        }

        var accessToken = await GetAccessTokenAsync(cancellationToken);
        var notebookName = await CreateNotebookAsync(accessToken, notebookTitle, cancellationToken);

        try
        {
            var sourceNames = await UploadSourceAsync(accessToken, notebookName, sourceText, cancellationToken);
            return await QueryNotebookAsync(accessToken, notebookName, sourceNames, prompt, cancellationToken);
        }
        finally
        {
            // Aufräumen: pro Aufruf wird ein Wegwerf-Notebook angelegt, das danach nicht mehr
            // gebraucht wird. Fehler beim Löschen sollen den eigentlich schon erfolgreichen
            // Aufruf nicht nachträglich als Fehlschlag erscheinen lassen.
...
#pragma warning restore CS0618
    ...
    /// </summary>
    private async Task<string> CreateNotebookAsync(string accessToken, string title, CancellationToken cancellationToken)
    {
    ...
    /// </summary>
    private async Task<IReadOnlyList<string>> UploadSourceAsync(string accessToken, string notebookName, string sourceText, CancellationToken cancellationToken)
    {
        ...
        var request = new HttpRequestMessage(HttpMethod.Post, url)
        {
            Content = JsonContent.Create(new
            {
                userContents = new[]
                {
                    new { textContent = new { sourceName = "LernTor-Import", content = sourceText } }
                }
            })
        };
        request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", accessToken);
    ...
    /// überprüft werden.
    /// </summary>
    private async Task<string> QueryNotebookAsync(string accessToken, string notebookName, IReadOnlyList<string> sourceNames, string prompt, CancellationToken cancellationToken)
    {
    ...
    private async Task TryDeleteNotebookAsync(string accessToken, string notebookName, CancellationToken cancellationToken)
    {
        try
        {
            ...
            // aber kein Grund, den ansonsten erfolgreichen Aufruf scheitern zu lassen.
        }
        catch (Exception)
        {
